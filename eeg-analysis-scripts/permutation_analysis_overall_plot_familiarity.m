...
% This is the fifth step (condition 'a'): plotting FAMvsCR comparison. Need
% to have stat_FAMvsCR_38subjs, dataCR, alltimelockCR, and alltimelockFAM
...
sample_count = length(stat_FAMvsCR_38subjs.time);
j = [0.5:timestep:0.6];   % Temporal endpoints (in seconds) of the ERP average computed in each subplot
m = [1:timestep*sampling_rate:sample_count];  % Temporal endpoints in EEG samples
% get relevant (significant) values
pos_cluster_pvals = [stat_FAMvsCR_38subjs.posclusters(:).prob];
neg_cluster_pvals = [stat_FAMvsCR_38subjs.negclusters(:).prob];

pos_signif_clust = find(pos_cluster_pvals < stat_FAMvsCR_38subjs.cfg.alpha);
neg_signif_clust = find(neg_cluster_pvals < stat_FAMvsCR_38subjs.cfg.alpha);
pos = ismember(stat_FAMvsCR_38subjs.posclusterslabelmat, pos_signif_clust);
neg = ismember(stat_FAMvsCR_38subjs.negclusterslabelmat, neg_signif_clust);

% First ensure the channels to have the same order in the average and in the statistical output.
[i1,i2] = match_str(GA_FAMvsCR.label, stat_FAMvsCR_38subjs.label);
