...
for isub = 1:length(subjects)
    ...
    for iset = 1:config.num_blocks
        ...
        if~(num_behav_trials == num_eeg_epochs)
            fprintf('Number of ERP epochs (%d) does not match behavioral trials (%d) - skipping.\n',num_eeg_epochs, num_behav_trials)
            continue;
        end
        ...
        fprintf('Subject %s has %d recollected trials and %d familiar trials for ERPs before removal of epochs.\n', subj_str, num_rec_trials, num_fam_trials)
