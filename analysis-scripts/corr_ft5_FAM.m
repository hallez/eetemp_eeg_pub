...
% This is the fifth step (condition 'a'): plotting significant clusters for
% the ERPs from FAM trials.
...
[i1,i2] = match_str(timelockFAM.label, stat.label);
...
for k = 1:39;
   ...
   cfg = [];
   cfg.xlim=[j(k) j(k+1)];
   pos_int = zeros(numel(timelockFAM.label),1);
   neg_int = zeros(numel(timelockFAM.label),1);
